...
/**
 * Created by Tsultrim on 6/11/18.
 */
    ...
    private static List<NewsFeed> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < newsArray.length(); i++) {
                ...
                String subject = currentNews.getString("webTitle");
                String url = currentNews.getString("webUrl");

                Date publicationDate = null;
                try {
                    publicationDate = (new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss")).parse(date);
                } catch (Exception e) {

                    Log.e("QueryUtils", "Problem parsing the news date", e);
                }

                NewsFeed newsFeed = new NewsFeed(section, publicationDate, subject, url);
